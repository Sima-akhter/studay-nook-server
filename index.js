...
const run = async () => {
  try {
    ...
    const authenticate = catchAsync(async (req, res, next) => {
      // 1. Try verifying via Next.js Better Auth endpoint
      // This ensures 100% compatibility with Better Auth's token hashing/parsing logic
      try {
        ...
        // Fallback to manual if frontend is unreachable
        console.error("Auth fetch failed, falling back to DB:", err.message);
      }

      // 2. Fallback to manual token lookup in DB
      ...
      if (
        ...
        userQuery = {
          $or: [{ _id: session.userId }, { _id: new ObjectId(session.userId) }],
        };
      }
    ...
    app.get("/rooms", catchAsync(async (req, res) => {
      const { search, amenities, floor, minRate, maxRate, sort, order, page = 1, limit = 10 } = req.query;
    ...
    // GET /rooms/latest
    app.get("/rooms/latest", catchAsync(async (req, res) => {
    ...
    app.get("/rooms/my-listings", authenticate, catchAsync(async (req, res) => {
      const rooms = await roomsCollection.find({ owner: req.user._id.toString() }).toArray();
      res.json({ success: true, data: rooms });
    }));

    // GET /rooms/availability
    app.get("/rooms/availability", catchAsync(async (req, res) => {
      const { roomId, date } = req.query;
      if (!roomId || !date) return res.status(400).json({ success: false, message: "roomId and date required" });

      const bookings = await bookingsCollection.find({ roomId, date, status: "confirmed" }).toArray();
      const allSlots = [];
      for (let i = 8; i < 20; i++) {
    ...
    // POST /rooms
    app.post("/rooms", authenticate, catchAsync(async (req, res) => {
      const room = req.body;
      ...
      if (
        ...
        room.hourlyRate === undefined || room.hourlyRate === null
      ) {
      ...
      if (isNaN(room.floor) || isNaN(room.capacity) || isNaN(room.hourlyRate)) {
        return res.status(400).json({ success: false, message: "Invalid numeric fields provided" });
      }
    ...
    // PATCH /rooms/:id
    app.patch("/rooms/:id", authenticate, catchAsync(async (req, res) => {
      const id = req.params.id;
      if (!ObjectId.isValid(id)) return res.status(400).json({ success: false, message: "Invalid ID" });
      ...
      if (updates.floor !== undefined && updates.floor !== null) {
        ...
        if (isNaN(updates.floor)) return res.status(400).json({ success: false, message: "Invalid numeric floor" });
      }
      if (updates.capacity !== undefined && updates.capacity !== null) {
        updates.capacity = parseInt(updates.capacity);
        if (isNaN(updates.capacity)) return res.status(400).json({ success: false, message: "Invalid numeric capacity" });
      }
      if (updates.hourlyRate !== undefined && updates.hourlyRate !== null) {
        updates.hourlyRate = parseFloat(updates.hourlyRate);
        if (isNaN(updates.hourlyRate)) return res.status(400).json({ success: false, message: "Invalid numeric hourlyRate" });
      }
      ...
      res.json({ success: true, message: "Room updated successfully", data: result });
    }));

    // DELETE /rooms/:id
    app.delete("/rooms/:id", authenticate, catchAsync(async (req, res) => {
      const id = req.params.id;
      if (!ObjectId.isValid(id)) return res.status(400).json({ success: false, message: "Invalid ID" });
      ...
      res.json({ success: true, message: "Room deleted successfully", data: result });
    }));
    ...
    // POST /bookings
    app.post("/bookings", authenticate, catchAsync(async (req, res) => {
      ...
      if (overlap) {
        return res.status(409).json({ success: false, message: "Booking conflict detected" });
      }
    ...
    app.get("/bookings/my-bookings", authenticate, catchAsync(async (req, res) => {
    ...
    app.patch("/bookings/:id/cancel", authenticate, catchAsync(async (req, res) => {
      const id = req.params.id;
      if (!ObjectId.isValid(id)) return res.status(400).json({ success: false, message: "Invalid ID" });
      ...
      if (booking.userId !== req.user._id.toString()) {
        return res.status(403).json({ success: false, message: "Only booking owner can cancel" });
      }
    ...
    app.get("/bookings/room/:roomId", catchAsync(async (req, res) => {
      const roomId = req.params.roomId;
      const bookings = await bookingsCollection.find({ roomId, status: { $ne: "cancelled" } }).toArray();
      res.json({ success: true, data: bookings });
    }));

    // GET /bookings/:id
    app.get("/bookings/:id", authenticate, catchAsync(async (req, res) => {
      const id = req.params.id;
      if (!ObjectId.isValid(id)) return res.status(400).json({ success: false, message: "Invalid ID" });
    ...
    app.get("/dashboard/summary", authenticate, catchAsync(async (req, res) => {
      const userId = req.user._id.toString();

      const totalRoomsOwned = await roomsCollection.countDocuments({ owner: userId });
      const totalBookingsMade = await bookingsCollection.countDocuments({ userId });
      const totalConfirmedBookings = await bookingsCollection.countDocuments({ userId, status: "confirmed" });
      const totalCancelledBookings = await bookingsCollection.countDocuments({ userId, status: "cancelled" });
    ...
    app.get("/dashboard/recent-bookings", authenticate, catchAsync(async (req, res) => {
      const bookings = await bookingsCollection.aggregate([
        { $match: { userId: req.user._id.toString() } },
        { $sort: { createdAt: -1 } },
        { $limit: 5 },
    ...
    app.get("/stats", catchAsync(async (req, res) => {
      ...
      const activeBookings = await bookingsCollection.countDocuments({ status: "confirmed" });
